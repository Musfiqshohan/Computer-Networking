...
class SendThread3 extends Thread{
    ...
    SendThread3(String mynode, String sendToPort)
    {
        try {
    ...
    public void run()
    {
        ...
        try {
            ...
            while(running) {
                ...
                String sendData = "";
                for (int j = 0; j < 10; j++) {
                    sendData = Server3.myrouter.routingTable[j].retNodeInfo();
...
public class Server3 extends  Thread{
    ...
    public Server3(int port) throws Exception {
    ...
    public static void main(String[] args) throws  Exception {
        ...
        prepareRoutingTable("configC.txt");



        client=new EchoClient();

        Server3 server= new Server3(Integer.valueOf(myport));
        server.start();

        ///Under Construction

        Thread.sleep(5000);
        SendThread3 sendThread[]= new SendThread3[15];
        for(int i=0;i<neighbour.size();i++)  //neighbour.size()
        {
            sendThread[i]=new SendThread3(mynode, neighbour.get(i).port);
            sendThread[i].start();
